...
while True:
	...
	if key == ord(" "):
		...
		for (startX, startY, endX, endY) in boxes:
			...
			# get the text from the box image with a margin of 10 pixels
			roi = orig[startY - 10:endY + 10, startX - 10:endX + 10]
			gray = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)
			text = pytesseract.image_to_string(gray, lang='por', config='--psm 3 --oem 3')
			print(text)
			text = "".join([c if ord(c) < 128 else "" for c in text]).strip()

			# draw the text on the frame
			cv2.putText(orig, text, (startX, startY - 10), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 0, 255), 3)
			# draw rectangle with margin of 10 pixels in blue color
			cv2.rectangle(orig, (startX - 10, startY - 10), (endX + 10, endY + 10), (255, 0, 0), 2)

		# show the output frame
		test += 1
		cv2.imshow("Photo Text Detection " + test.__str__(), orig)
	if key == ord("q"):
		break
